<template>
    <div id="shortVideoManager">
      <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
        <el-tab-pane label="短视频列表" name="first">
          <el-form :inline="true" :model="shortVideoForm" class="demo-form-inline">
            <el-form-item label="帐号">
              <el-input v-model="shortVideoForm.username" placeholder="帐号"></el-input>
            </el-form-item>
            <el-form-item label="状态">
              <el-select v-model="shortVideoForm.status" placeholder="状态">
                <el-option label="全部" value="2"></el-option>
                <el-option label="已禁用" value="0"></el-option>
                <el-option label="投放中" value="1"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="上传时间">
              <el-date-picker
                v-model="shortVideoForm.from_time"
                type="date"
                placeholder="选择日期时间"
                value-format="yyyy-MM-dd"
              >
              </el-date-picker>至
              <el-date-picker
                v-model="shortVideoForm.end_time"
                type="date"
                value-format="yyyy-MM-dd"
                placeholder="选择日期时间">
              </el-date-picker>
            </el-form-item>
            <el-form-item>
              <el-button type="info" @click="getShortVideoList(1)">查询</el-button>
            </el-form-item>
          </el-form>
          <div class="tablePanel">
              <el-button class="btn btn-default" @click="showModal(1)">批量投放</el-button>
              <el-button  class="btn btn-default" @click="showModal(0)">批量禁用</el-button>
          </div>
          <el-table
            ref="multipleTable"
            :data="shortVideoData"
            tooltip-effect="dark"
            border
            style="width: 100%"
            @selection-change="handleSelectionChange">
            <el-table-column
              type="selection"
              header-align="center"
              width="55">
            </el-table-column>
            <el-table-column
              header-align="center"
              width="310"
              label="视频信息">
              <template slot-scope="scope">
                <div class="videoBlock p10">
                  <p class="videoBlockTitle">
                    {{scope.row.title}}
                  </p>
                  <div class="videoBlockLeft m-r15 f-l">
                    <img :src="scope.row.cover"/>
                    <span class="videoTime">{{scope.row.length | filterTime}}</span>
                  </div>
                  <div class="videoBlockRight f-l">
                    <p class="videoBlockDataTime alignLeft opacity8"><span>{{scope.row.create_time}}</span></p>
                    <p class="videoBlockOther alignLeft opacity8">
                      <span class="videoBlockPlayNum disInlineB w-85"><i class="glyphicon"></i><span> {{scope.row.thumbsup_number}} </span></span>
                      <span class="videoBlockCommentNum disInlineB w-85"><i class="glyphicon"></i><span> {{scope.row.remark_count}} </span></span>
                    </p>
                  </div>
                </div>
              </template>
            </el-table-column>
            <el-table-column
              label="商品数"
              header-align="center"
              >
              <template slot-scope="scope">
                <p>{{scope.row.goods_count}}</p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              label="实付订单">
              <template slot-scope="scope">
                <p>数量：<span> {{scope.row.order_number}} </span></p>
                <p>总额：<span> ¥ {{scope.row.order_amount}} </span></p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              label="红包打赏">
              <template slot-scope="scope">
                <p>数量：<span> {{scope.row.redenvelope_num}} </span></p>
                <p>总额：<span> ¥ {{scope.row.redenvelope_amount}} </span></p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              prop="owner"
              label="帐号">
            </el-table-column>
            <el-table-column
              header-align="center"
              label="链接">
              <template slot-scope="scope">
                <p><a :href="scope.row.url" target="_blank">{{scope.row.url}}</a></p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              label="操作">
              <template slot-scope="scope">
                <el-button  type="primary" size="mini" @click="recommend(scope.row.id,1)" v-if="scope.row.is_recommend==0" :id="scope.row.id">推荐到小程序</el-button>
                <el-button  type="primary" size="mini" @click="recommend(scope.row.id,0)"  v-else>取消推荐</el-button>
                <el-button  type="info" size="mini" @click="showModal(1,scope.row.id)" v-if="scope.row.status==0" >投放</el-button>
                <el-button  type="danger" size="mini" @click="showModal(0,scope.row.id)"  v-else>禁用</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
            @current-change="getShortVideoList"
            :page-size="pagination.perSize"
            layout="total, prev, pager, next"
            :total="pagination.total">
          </el-pagination>
        </el-tab-pane>
        <el-tab-pane label="小程序推荐列表" name="second">
          <el-form :inline="true" :model="appForm" class="demo-form-inline">
            <el-form-item label="帐号">
              <el-input v-model="appForm.username" placeholder="帐号"></el-input>
            </el-form-item>
            <el-form-item label="上传时间">
              <el-date-picker
                v-model="appForm.from_time"
                type="date"
                placeholder="选择日期时间"
                value-format="yyyy-MM-dd"
              >
              </el-date-picker>至
              <el-date-picker
                v-model="appForm.end_time"
                type="date"
                value-format="yyyy-MM-dd"
                placeholder="选择日期时间">
              </el-date-picker>
            </el-form-item>
            <el-form-item>
              <el-button type="info" @click="getappList(1)">查询</el-button>
            </el-form-item>
          </el-form>
          <el-table
            ref="multipleTable"
            :data="appListData"
            tooltip-effect="dark"
            border
            style="width: 100%"
           >
            <el-table-column
              header-align="center"
              width="310"
              label="视频信息">
              <template slot-scope="scope">
                <div class="videoBlock p10">
                  <p class="videoBlockTitle">
                    {{scope.row.title}}
                  </p>
                  <div class="videoBlockLeft m-r15 f-l">
                    <img :src="scope.row.cover"/>
                    <span class="videoTime">{{scope.row.length | filterTime}}</span>
                  </div>
                  <div class="videoBlockRight f-l">
                    <p class="videoBlockDataTime alignLeft opacity8"><span>{{scope.row.create_time}}</span></p>
                    <p class="videoBlockOther alignLeft opacity8">
                      <span class="videoBlockPlayNum disInlineB w-85"><i class="glyphicon"></i><span> {{scope.row.thumbsup_number}} </span></span>
                      <span class="videoBlockCommentNum disInlineB w-85"><i class="glyphicon"></i><span> {{scope.row.remark_count}} </span></span>
                    </p>
                  </div>
                </div>
              </template>
            </el-table-column>
            <el-table-column
              label="商品数"
              header-align="center"
              width="120">
              <template slot-scope="scope">
                <p>{{scope.row.goods_count}}</p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              label="实付订单">
              <template slot-scope="scope">
                <p>数量：<span> {{scope.row.order_number}} </span></p>
                <p>总额：<span> ¥ {{scope.row.order_amount}} </span></p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              label="红包打赏">
              <template slot-scope="scope">
                <p>数量：<span> {{scope.row.redenvelope_num}} </span></p>
                <p>总额：<span> ¥ {{scope.row.redenvelope_amount}} </span></p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              prop="owner"
              label="帐号">
            </el-table-column>
            <el-table-column
              header-align="center"
              label="链接">
              <template slot-scope="scope">
                <p><a :href="scope.row.url" target="_blank">{{scope.row.url}}</a></p>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              width="120"
              label="排序">
              <template slot-scope="scope">
                <el-input size="mini" style="width:80px;" @blur="appSort(scope.row.id,scope.row.sort)" v-model="scope.row.sort" ></el-input>
              </template>
            </el-table-column>
            <el-table-column
              header-align="center"
              label="操作">
              <template slot-scope="scope">
                <el-button  type="primary" size="mini" @click="recommend(scope.row.id,0)">移除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
            @current-change="getappList"
            :page-size="pagination.perSize"
            layout="total, prev, pager, next"
            :total="pagination.total">
          </el-pagination>
        </el-tab-pane>
      </el-tabs>
    </div>
</template>

<script>
    export default {
        name: "shortVideoManager",
      data(){
          return{
            activeName:'first',
            shortVideoForm:{
              username:'',
              status:'',
              from_time:'',
              end_time:'',
            },
            appForm:{
              username:'',
              from_time:'',
              end_time:''
            },
            pagination:{
              page:1,
              total:0,
              perSize:10
            },
            shortVideoData:[],
            shortVideoSelect:[],
            appListData:[],
          }
      },
      created(){
        this.getShortVideoList(1);
      },
      filters:{
        filterTime:(val)=>{
          if (val < 86400) {
            var dt = new Date("01/01/2000 0:00");
            dt.setSeconds(val);
            var h = dt.getHours(),
              m = dt.getMinutes(),
              s = dt.getSeconds(),
              r = "";
            // if (h > 0) {
            r += (h > 9 ? h.toString() : "0" + h.toString()) + ":";
            // }
            r += (m > 9 ? m.toString() : "0" + m.toString()) + ":"
            r += (s > 9 ? s.toString() : "0" + s.toString());
            return r;
          } else {
            return null;
          }
        },
      },
      methods:{
        //选项卡切换触发事件
        handleClick(tab){
          this.pagination={
            page:1,
            total:0,
            perSize:10
          };
          this.shortVideoSelect=[];
          if(tab.name=='first'){
            this.getShortVideoList(1);
          }else{
            this.getappList(1);
          }
        },
        //获取短视频列表
        getShortVideoList(page){
          var smData={};
          smData.page=page;
          smData.username=this.shortVideoForm.username;
          smData.status=this.shortVideoForm.status;
          smData.from_time=this.shortVideoForm.from_time;
          smData.end_time=this.shortVideoForm.end_time;
          smData.act=11;
          this.api.request({
            url:this.api.admin_short_video,
            method: 'post',
            data:smData,
          }).then((res)=>{
            this.shortVideoData=res.list;
            this.pagination.page=page;
            this.pagination.total=res.total_count;
          })
        },
        //短视频 列表checkbox事件
        handleSelectionChange(val){
          this.shortVideoSelect = val;
        },
        //推荐到小程序
        recommend(id,flag){
          var smData={
            act:11,
            svid:id,
            is_recommend:flag
          };
          this.api.request({
            url:this.api.recommend_short_video,
            method:'post',
            data:smData
          }).then((res)=>{
            if(flag==0){
              var message='取消推荐成功！'
            }else{
              var message='推荐成功！'
            }
            this.$message({
              type:'success',
              message:message
            });
            if(this.activeName=='first'){
              this.getShortVideoList(this.pagination.page);
            }else{
              this.getappList(this.pagination.page);
            }
          });
        },
        //投放 禁用request
        //批量投放，禁用触发事件
        showModal(flag,svids){
          if(svids==undefined||svids==null||svids==''){
            var ids=[]
            if(this.shortVideoSelect.length>0){
              this.shortVideoSelect.forEach(item=>{
                ids.push(item.id);
              })
              this.showModalRequset(flag,ids.join(','))
            }else{
              this.$message.error('请先选择视频！')
            }
          }else{
            this.showModalRequset(flag,svids)
          }


        },
        showModalRequset(flag,svids){
          if(flag==1){
            var text="确定投放该小程序么"
          }else{
            var text="确定禁用该小程序么"
          }
          this.$confirm(text, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            var smData={
              act:11,
              svid:svids,
              status:flag
            };
            this.api.request({
              url:this.api.updown_short_video,
              method: 'post',
              data: smData
            }).then((res)=>{
              this.shortVideoSelect=[];
              this.getShortVideoList(this.pagination.page);
              this.$message({
                type:'success',
                message:'操作成功！'
              });
            });
          }).catch();
        },
        //小程序推荐列表
        getappList(page){
          var smData={
            act:11,
            from_time:this.appForm.from_time,
            end_time:this.appForm.end_time,
            username:this.appForm.username,
            page:page
          }
          this.api.request({
            url:this.api.admin_short_video_recommend,
            method: 'post',
            data:smData
          }).then((res)=>{
            this.appListData=res.list;
            this.pagination.page=page;
            this.pagination.total=res.total_count;
          });
        },
        //小程序推荐排序
        appSort(id,sort){
          var smData={
            act:11,
            svid:id,
            sort:sort
          }
          this.api.request({
            url:this.api.recommend_sort,
            method:'post',
            data:smData
          }).then((res)=>{
            this.$message({
              type:'success',
              message:'排序成功！'
            });
            this.getappList(this.pagination.page);
          });
        }
      },
    }
</script>

<style lang="less" scoped>
  #shortVideoManager{
    .videoBlock{
      display: block;
      width:280px;
    }
    .tablePanel{
      padding: 10px;
      background: #fff;
      border-bottom: 1px solid #e5e5e5;
      text-align: left;
    }
    .alignLeft{
      text-align: left;
    }
    .videoBlockLeft{
      width:90px;
      height:70px;
      overflow: hidden;
      position: relative;
    }
    .videoBlockTitle{
      overflow: hidden;
      text-overflow: ellipsis;
      white-space: nowrap;
      width:240px;
      font-weight:bold;
      margin-bottom:10px;
      text-align: left;
    }
    .videoBlockLeft .videoTime{
      display:inline-block;
      height:20px;
      line-height: 20px;
      width:90px;
      background: rgba(0, 0, 0, 0.5);
      color:#fff;
      position:absolute;
      bottom:0px;
      left:0px;
      text-align: center;

    }
    .w-85{
      width:85px;
      display: inline-block;
    }
    .videoBlockLeft img{
      width:90px;
      height:100%;
    }
    .videoBlockRight{
      width:190px;
      padding-left:15px;
    }
    .videoBlockPlayNum i{
      background: url(data:image/png;base64,) no-repeat left center;
      background-size: 20px 20px;
      padding:10px 10px;
    }
    .videoBlockCommentNum i{
      background: url(data:image/png;base64,) no-repeat left center;
      background-size: 20px 20px;
      padding: 10px 10px;
    }
  }

</style>
